...
def main():
    ...
    def update():
        #Updates changes to plot displayed on server        
        networkx.set_node_attributes(g, node_color_dict, 'node_color') #changes node attributes in networkx
        color_attribute= 'node_color'
        networkx.set_node_attributes(g, outline_dict, 'border_color') #changes node attributes in networkx
        node_outline= 'border_color'
        networkx.set_edge_attributes(g, bColor, 'branch_color')  #Changes branch color attribute
        edge_cmap= 'branch_color'

        newGraph= from_networkx(g, nx.kamada_kawai_layout, scale=30, center=(0, 0)) #created updated graph object

        #Applies hovering to the new graph object
        hover_nodes = HoverTool(
                        tooltips= [("Node", "@index"), ("Base Voltage(V)", "@node_base_v"), ("Phase", "@node_phase"), ("From Node", "@fromNode")],
                        renderers= [newGraph.node_renderer]
                        )

        hover_edges = HoverTool(
	    			    tooltips=[('Branch', '@label'), ('Type','@type'), ("Phase", "@phase"), ("From", "@from"), ("To", "@to"), ("Length", "@length")],
		    		    renderers=[newGraph.edge_renderer],
                        line_policy= 'interp'
				        )

        #Adds data to graph objects
        newGraph.node_renderer.glyph= Circle(size=15, fill_color= color_attribute, line_color= node_outline)
        newGraph.edge_renderer.glyph = MultiLine(line_alpha=0.5, line_width=1, line_color= edge_cmap)

        #Updates visual tool on Bokeh Server
        plot.tools[4] = hover_nodes
        plot.tools[5] = hover_edges        
        
        plot.renderers[0]= newGraph
